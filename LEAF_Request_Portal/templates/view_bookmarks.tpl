...
<div id="bookmark_<!--{$record.recordID|strip_tags}-->" style="float: left; border: 1px solid black; margin: 8px; padding: 0px; background-color: <!--{$record.stepBgColor|strip_tags}-->; color: <!--{$record.stepFontColor|strip_tags}-->; width: 370px">
    <div style="float: left; cursor: pointer; background-color: black; font-size: 150%; font-weight: bold; color: white; padding: 8px; text-align: center" onclick="window.location='?a=printview&amp;recordID=<!--{$record.recordID|strip_tags}-->'" role="button" tabindex="0" onkeypress="if (event.keyCode==13){ window.location='?a=printview&amp;recordID=<!--{$record.recordID|strip_tags}-->'}">
        <!--{$record.recordID}--><br />
        <!--{if $record.actionIcon != ''}-->
        <img src="../libs/dynicons/?img=<!--{$record.actionIcon|strip_tags}-->&amp;w=32" alt="<!--{$record.stepTitle|strip_tags}--> <!--{$record.actionTextPasttense|strip_tags}-->" title="<!--{$record.stepTitle|strip_tags}--> <!--{$record.actionTextPasttense|strip_tags}-->" />
        <!--{else}-->
        <img src="../libs/dynicons/?img=emblem-notice.svg&amp;w=32" alt="<!--{$record.stepTitle|strip_tags}--> <!--{$record.actionTextPasttense|strip_tags}-->" title="<!--{$record.stepTitle|strip_tags}--> <!--{$record.actionTextPasttense|strip_tags}-->" />
        <!--{/if}-->
    </div>
    <div>
        <div style="background-color: #e0e0e0; font-weight: bold; border-bottom: 1px solid black; padding: 2px">
                        <span style="float: right; cursor: pointer" tabindex="-1"><img src="../libs/dynicons/?img=process-stop.svg&amp;w=16" alt="Delete Bookmark" title="Delete Bookmark" onclick="removeBookmark(<!--{$record.recordID|strip_tags}-->)" role="button" tabindex="0" onkeypress="if (event.keyCode==13){ removeBookmark(<!--{$record.recordID|strip_tags}-->)}"/></span>
